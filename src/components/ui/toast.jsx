...
const toastVariants = {
  default: 'border border-border bg-white text-text-primary',
  success: 'border-success/30 bg-green-50 text-green-900',
  destructive: 'border-error/30 bg-red-50 text-red-900',
  warning: 'border-warning/30 bg-orange-50 text-orange-900',
  info: 'border-accent/30 bg-blue-50 text-blue-900',
};
...
const ToastAction = React.forwardRef(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      'inline-flex h-8 shrink-0 items-center justify-center rounded-lg border border-border bg-transparent px-3 text-sm font-medium transition-colors hover:bg-gray-100 focus:outline-none disabled:pointer-events-none disabled:opacity-50',
      className
    )}
    {...props}
  />
...
const ToastClose = React.forwardRef(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      'absolute right-2 top-2 rounded-lg p-1 text-text-muted opacity-0 transition-opacity hover:text-text-primary focus:opacity-100 focus:outline-none group-hover:opacity-100',
      className
    )}
    toast-close=""
    {...props}
